...
1. List all your remote repositories and show their URLs:
   ...
   OUTPUT would be 
   origin  https://github.com/GGWPXXXX/git-commands.git (fetch)
   origin  https://github.com/GGWPXXXX/git-commands.git (push)
   ```
...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > This error occured when you try to push local repository but you not yet connected to git remote repository.
